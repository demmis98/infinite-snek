...
int main(){
  ...
  SDL_Window *window = SDL_CreateWindow("infinite snek", 64, 64, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    ...
    if(!dead){
      ...
      if(time_new - time_old > 1000/20){
        ...
        for(int i = 0; i < player_l; i++){
          ...
          if(temp_x == -1)
            temp_x = world_width - 1;
          else if(temp_x == world_width)
            temp_x = 0;
          if(temp_y == -1)
            temp_y = world_height - 1;
          else if(temp_y == world_height)
            temp_y = 0;
        }
